...
6. **Never cut inside a word.** Snap every cut edge to a word boundary from the ASR transcript.
...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached normalized Doubao ASR JSON
...
- `DOUBAO_API_KEY` resolves — either in the environment or in `.env` at the video-use repo root. If missing, ask the user to paste a new-console Doubao Speech App Key and write it to `.env` (never to the user's `<videos_dir>`).
...
- **`transcribe.py <video>`** — single-file Doubao BigModel ASR call with word timestamps and automatic speaker diarization. Cached. `--check-key` validates access without transcribing audio.
...
- **Audio events as signals, when available.** `(laughs)`, `(sighs)`, `(applause)` mark beats. Doubao's turbo endpoint does not currently tag these events, so never invent them.
...
- **Running Whisper locally on CPU.** Slow and it normalizes fillers. Use the configured word-level Doubao ASR endpoint.
