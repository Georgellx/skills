"""Pack normalized ASR transcripts in <edit>/transcripts/ into readable markdown.
...
hour of takes in a tenth the tokens of raw ASR JSON and gives
...
def _join_tokens(tokens: list[str]) -> str:
    """Join English word tokens and CJK character tokens without visual gaps."""
    if not tokens:
        return ""
    text = tokens[0]
    no_space_before = set(",.!?;:%)]}，。！？；：、）】》」』")
...
def group_into_phrases(
    ...
    """Walk a normalized word list, break into phrases on silence >= threshold
    OR speaker change. Returns list of {start, end, text, speaker_id}.

    `words` entries may have types 'word', 'spacing', or 'audio_event'.
...
def render_markdown(entries: list[tuple[str, float, list[dict]]], silence_threshold: float) -> str:
    ...
    for name, duration, phrases in entries:
        ...
        for p in phrases:
            ...
            if spk is not None:
                # Providers return IDs like "speaker_0" — strip the prefix for readability
                spk_str = str(spk)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Pack ASR transcripts into takes_packed.md")
