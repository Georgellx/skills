...
description: Install video-use into the current agent (Claude Code, Codex, Hermes, Openclaw, etc.) and wire up ffmpeg + the Doubao Speech API key so the user can start editing immediately.
---
...
3. A new-console Doubao Speech App Key in `.env` at the repo root (for BigModel ASR transcription).
...
- Do everything yourself. Only ask the user for things you cannot generate — the Doubao Speech App Key, and confirmation before `brew install`.
...
### 5. Doubao Speech API key

Doubao BigModel ASR does all transcription. Without a new-console App Key, nothing transcribes.

1. Check existing state in this order and stop at the first hit:

    ```bash
    # a) env var already exported
    [ -n "$DOUBAO_API_KEY" ] && echo "env"
    # b) .env at repo root already has it
    grep -q '^DOUBAO_API_KEY=..' ~/Developer/video-use/.env 2>/dev/null && echo "dotenv"
    ```

2. If neither is set, ask the user exactly once:

    > I need a new-console Doubao Speech App Key for transcription (word-level timestamps and speaker diarization). Paste it here and I'll write it to `~/Developer/video-use/.env`. Or if you already have it exported as `DOUBAO_API_KEY`, say "use env" and I'll skip.

    When the user pastes a key, write it to `~/Developer/video-use/.env`:

    ```bash
    printf 'DOUBAO_API_KEY=%s\n' "$KEY" > ~/Developer/video-use/.env
    chmod 600 ~/Developer/video-use/.env
    ```

    Never echo the key back in tool output. Never commit `.env`.

3. Sanity check with an empty-audio request that consumes no transcription quota:

    ```bash
    python ~/Developer/video-use/helpers/transcribe.py --check-key
    ```

    An expected empty-audio validation response means the key and `volc.bigasr.auc_turbo` route are available. Authentication or resource errors mean the key is invalid or the service has not been enabled in the Doubao Speech console.
...
Full transcription is optional at install time because it consumes ASR quota. Better to wait until the user hands you their first clip.
...
- Never run transcription as part of install verification unless the user explicitly asks — Doubao ASR consumes quota.
